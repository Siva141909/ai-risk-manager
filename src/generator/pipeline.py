...
from src.generator.entity_assignment import PoolRatios, assign_entities
...
class GeneratorConfig:
    pool_ratios: PoolRatios = field(default_factory=PoolRatios)
...
def run_generator(df: pd.DataFrame, seed: int, config: GeneratorConfig = GeneratorConfig()) -> GeneratorResult:
    assigned = assign_entities(df, seed, config.pool_ratios)
    ...
    with_rings, ring_records, _ = inject_rings(
        with_clusters, seed, config.ring_types, config.decoy_attach_probability, used_by_clusters
    )
